...
const ensureInstructor = async (context: functions.https.CallableContext) => {
...
export const analyzeEquipmentCondition = functions.runWith({ secrets: [geminiApiKey] }).https.onCall(async (data, context) => {
    ensureAuthenticated(context);
...
export const readInventoryLabel = functions.runWith({ secrets: [geminiApiKey] }).https.onCall(async (data, context) => {
    ensureAuthenticated(context);
    const { photoB64 } = data;
...
export const generateLoanReportAnalysis = functions.runWith({ secrets: [geminiApiKey] }).https.onCall(async (data, context) => {
...
export const generateMaintenanceSuggestions = functions.runWith({ secrets: [geminiApiKey] }).https.onCall(async (data, context) => {
    ensureInstructor(context);
